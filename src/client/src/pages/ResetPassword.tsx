    ...
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden flex-col justify-between p-12">
        ...
        <div className="relative z-10 space-y-6">
          <div>
            <h2 className="text-4xl font-bold text-foreground leading-tight">Your Biology{"\n"}Doesn't Lie.</h2>
            <p className="mt-3 text-foreground/60 text-base leading-relaxed">
          ...
          <div className="space-y-3">
            {BULLETS.map((b, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center flex-shrink-0 text-primary">
                  {b.icon}
                </div>
        ...
        <div className="relative z-10 w-full max-w-sm">
          ...
          {done ? (
            <div className="text-center space-y-4">
              ...
              <p className="text-foreground/50 text-sm">Your password has been updated successfully.</p>
              <Button className="w-full bg-primary hover:bg-primary rounded-full font-semibold text-foreground mt-4" onClick={() => setLocation("/login")}>
                Sign in
              </Button>
            </div>
          ) : serverError && !token ? (
            <div className="text-center space-y-4">
              ...
              <p className="text-foreground/50 text-sm">{serverError}</p>
              <Button variant="outline" className="w-full border-foreground/10 bg-foreground/5 text-foreground hover:bg-foreground/10 mt-4" onClick={() => setLocation("/forgot-password")}>
                Request new link
              </Button>
            </div>
          ) : (
            <>
              <div className="mb-8">
                ...
                <p className="text-foreground/50 text-sm mt-1">Choose a strong password for your account</p>
              </div>
              ...
              <Form {...({ control } as any)}>
                <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <div className="relative">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30" />
                            <Input {...field} type="password" placeholder="••••••••" className="pl-10 bg-foreground/5 border-foreground/10 text-foreground placeholder:text-foreground/20 focus:border-primary/50" />
                          </div>
                        </FormControl>
                  ...
                  {serverError && <p className="text-sm text-primary text-center">{serverError}</p>}

                  <Button type="submit" disabled={mutation.isPending} className="w-full bg-primary hover:bg-primary rounded-full font-semibold text-foreground">
              ...
              <p className="text-center text-sm text-foreground/40 mt-6">
                <a href="/login" className="text-primary hover:text-primary">Back to sign in</a>
              </p>
            </>
          )}
        </div>
